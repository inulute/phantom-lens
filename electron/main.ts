...
function applyInteractivityState(): void {
  ...
  try {
    ...
    if (shouldBeInert) {
      // CRITICAL: Set skipTaskbar FIRST to prevent taskbar from appearing
      // Then set focusable to false to prevent focus stealing
      // Order matters: skipTaskbar must be set before other operations
      state.mainWindow.setSkipTaskbar(true);
      state.mainWindow.setFocusable(false);
      state.mainWindow.setIgnoreMouseEvents(true, { forward: true }); // Use forward option like og/main.ts
      // Do NOT call setAlwaysOnTop when inert - it can cause focus stealing
    } else {
      state.mainWindow.setIgnoreMouseEvents(false);
      state.mainWindow.setFocusable(true);
      state.mainWindow.setSkipTaskbar(true);
      // Only set always on top for initial view, and do it after setting focusable
...
function setWindowDimensions(width: number | string, height: number): void {
  ...
  // CRITICAL: Determine if window should be inert BEFORE the async operation
  // This ensures we can set focusable to false immediately when the timeout fires
  const shouldBeInert = state.mode === "stealth" || 
                       state.view === "response" || 
                       state.view === "followup";
  ...
  dimensionUpdateTimeout = setTimeout(() => {
    ...
    // CRITICAL: Set skipTaskbar FIRST, then set window to non-focusable IMMEDIATELY
    // This must happen BEFORE any bounds operations to prevent focus stealing and taskbar appearance
    if (shouldBeInert) {
      state.mainWindow.setSkipTaskbar(true); // Set FIRST to prevent taskbar from appearing
      state.mainWindow.setFocusable(false);
      state.mainWindow.setIgnoreMouseEvents(true, { forward: true }); // Use forward option like og/main.ts
    }
    ...
    try {
      ...
      // CRITICAL: For response/followup views, ensure skipTaskbar and non-focusable before ANY bounds operations
      // On Windows, setBounds can cause focus stealing and taskbar appearance even if window is non-focusable
      // So we use setSize and setPosition separately which are less likely to cause focus events
      if (shouldBeInert) {
        // Set skipTaskbar FIRST to prevent taskbar from appearing during resize
        state.mainWindow.setSkipTaskbar(true);
        // Then set focusable to false right before bounds change
        state.mainWindow.setFocusable(false);
        state.mainWindow.setIgnoreMouseEvents(true, { forward: true }); // Use forward option like og/main.ts
        
        // Use setSize and setPosition separately instead of setBounds
        // This prevents Windows from triggering focus events and taskbar appearance
        state.mainWindow.setSize(newBounds.width, newBounds.height);
        state.mainWindow.setPosition(newBounds.x, newBounds.y);
        
        // Ensure skipTaskbar is still set after resize operations
        state.mainWindow.setSkipTaskbar(true);
      } else {
        // For initial view, use setBounds normally
        state.mainWindow.setBounds(newBounds, false);
      }
...
function createWindow(): BrowserWindow {
  ...
  if (state.view === "response" || state.view === "followup" || state.mode === "stealth") {
    state.mainWindow.setFocusable(false);
    state.mainWindow.setIgnoreMouseEvents(true, { forward: true });
    state.mainWindow.setSkipTaskbar(true);
  }
